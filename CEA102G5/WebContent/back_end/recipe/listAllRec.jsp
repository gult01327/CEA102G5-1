...
<table>
	...
	<c:forEach var="recVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>" >
		<tr class='tabletr'>
			...
			<td>
			    <input type="button" id='verify' value="審核食譜" style="padding: 5px 10px; font-size:10px;">
			    <input type="hidden" id="recID" name="recID" value="${recVO.recID}">
				 <input type="button" id='reject' value="退回食譜" style="padding: 5px 10px; font-size:10px;">

			</td>
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back_end/recipe/rec.do" style="margin-bottom: 0px;">
			    <input type="submit" value="查看食譜明細" style="padding: 5px 10px; font-size:10px;">
