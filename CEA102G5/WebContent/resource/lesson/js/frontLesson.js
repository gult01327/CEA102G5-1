			...
			url : contextPath+"/lesson/les.do",
					...
					success : function(data) {
						for (let i = 0; i < data.length; i++) {
							...
							let picContainer = $("<div class=post-thumbnail><a><img src=" + contextPath + data[i].pic
									+ " height=100></a></div>");							
							let titleContainer = $("<div class=post-content></div>");
							let entryMeta = $("<div class=entry-meta><span class='posted-on'><i class='ion-calendar'></i><span>"+data[i].lesDate+"<div>"+data[i].lesTime+"</div></span></span><span>$"+data[i].lesPrice+"</span></div>");
