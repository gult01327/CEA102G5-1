	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			try {
				...
				if (lesID == null || (lesID.trim()).length() == 0) {
					errorMsgs.add("課程編號: 請勿空白");
				} else if (!lesID.trim().matches(enameReg)) { // 以下練習正則(規)表示式(regular-expression)
					errorMsgs.add("課程編號: 只能是數字");
				} else {
					lesrVO.setLesID(new Integer(lesID));
				}

				String memID = null;
				memID = req.getParameter("mem_ID").trim();
				if (memID == null || (memID.trim()).length() == 0) {
					errorMsgs.add("會員編號: 請勿空白");
				} else if (!memID.trim().matches(enameReg)) { // 以下練習正則(規)表示式(regular-expression)
					errorMsgs.add("會員編號: 只能是數字");
				} else {
					lesrVO.setMemID(new Integer(memID));
				}
				String lesrComments = null;
				lesrComments = req.getParameter("lesr_comments").trim();
				String lesrAnswer = null;
				lesrAnswer = req.getParameter("lesr_answer").trim();
				Boolean lesrStatus = new Boolean(req.getParameter("lesr_status").trim());
				String lesrReason = null;
				lesrReason = req.getParameter("lesr_reason").trim();
				...
				// Send the use back to the form, if there were errors
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("lesrVO", lesrVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back_end/reservation/addReservation.jsp");
					failureView.forward(req, res);
					return;
				}
				LesVO lesVO =new LesVO();
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/back_end/reservation/addReservation.jsp");
				failureView.forward(req, res);
			}
		}
